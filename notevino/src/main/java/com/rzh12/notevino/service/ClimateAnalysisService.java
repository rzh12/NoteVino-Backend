...
public class ClimateAnalysisService {
    ...
    public int analyzeFavoriteClimateByUserId(Integer userId) {
        // 1. Fetch all wine names and regions that the user has uploaded
        ...
        // If no data is found, return the default value 3 (Maritime Climate)
        if (wines.isEmpty()) {
            return 3;
        }

        // 2. Construct a string containing the name and region as the basis for analysis
    ...
    /**
     * Convert the climate type from the OpenAI response into the corresponding code
